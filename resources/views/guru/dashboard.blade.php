...
<div class="w-full space-y-6">
    ...
    {{-- WELCOME CARD --}}
    <div class="bg-gradient-to-br from-indigo-600 via-indigo-700 to-purple-700 rounded-2xl shadow-2xl p-8 text-white relative overflow-hidden">
        ...
        <div class="relative z-10">
            <div class="flex items-center gap-4 mb-4">
                ...
                <div>
                    ...
                    <p class="text-indigo-100 text-lg mt-1">{{ \Carbon\Carbon::now()->translatedFormat('l, d F Y') }}</p>
                </div>
            </div>
        ...
        <div class="p-6">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <form action="{{ route('guru_user.absen.masuk') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="w-full group relative overflow-hidden {{ $absensiHariIni ? 'bg-gray-300 cursor-not-allowed' : 'bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800' }} text-white font-bold py-4 px-6 rounded-xl shadow-lg transition-all duration-300 transform hover:scale-105 disabled:transform-none disabled:hover:scale-100"
                ...
                <form action="{{ route('guru_user.absen.pulang') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="w-full group relative overflow-hidden {{ !$absensiHariIni ? 'bg-gray-300 cursor-not-allowed' : 'bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800' }} text-white font-bold py-4 px-6 rounded-xl shadow-lg transition-all duration-300 transform hover:scale-105 disabled:transform-none disabled:hover:scale-100"
                        {{ !$absensiHariIni ? 'disabled' : '' }}>
                        <div class="flex items-center justify-center gap-3 relative z-10">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
